...
(function($) {
	...
	$.fn.jstree = function(settings) {
		...
		if (isMethodCall && settings.substring(0,
				1) == '_') { return returnValue; }

		// if a method call execute the method on all selected instances
		if (isMethodCall) {
			this.each(function() {
				var instance = instances[$.data(this, "jstree-instance-id")],
					methodValue = (instance && $.isFunction(instance[settings])) ? instance[settings].apply(
						instance,
						args) : instance;
			...
			this.each(function() {
				...
				$.each(instances[instance_id]._get_settings().plugins,
					function(i,
						val) { instances[instance_id].data[val] = {}; });
				$.each(instances[instance_id]._get_settings().plugins,
					function(i, val) {
						if (plugins[val]) {
	...
	$.jstree = {
		...
		_reference: function(needle) {
			...
			return instances[o.closest(".jstree")
			.data("jstree-instance-id")] || null;
		},
				...
				$.jstree._fn[i] = function() {
					...
					do {
						if (func && func.plugin && $.inArray(func.plugin,
								this._get_settings().plugins) !== -1) { break; }
						func = func.old;
					} while (func);
					if (!func) { return; }
	...
	$(function() {
		...
		var u = navigator.userAgent.toLowerCase(),
			...
			css_string = '' +
				...
				'.jstree a { display:inline-block; line-height:20px; height:20px; font-size: 18px; color:white; white-space:nowrap; text-decoration:none; padding:1px 2px; margin:0; } ' +
	...
	$.jstree.plugin("core", {
		__init: function() {
			this.data.core.to_open = $.map($.makeArray(this.get_settings().core.initially_open),
				function(n) {
					return "#" + n.toString()
					.replace(/^#/, "")
					.replace('\\/', '/')
		...
		_fn: {
			init: function() {
				...
				this.get_container()
				.html("<ul><li class='jstree-last jstree-leaf'><ins>&#160;</ins><a class='jstree-loading' href='#'><ins class='jstree-icon'>&#160;</ins>" + this._get_settings().core.strings.loading + "</a></li></ul>");
			...
			reopen: function(is_callback) {
				...
				if (this.data.core.to_open.length) {
					$.each(this.data.core.to_open, function(i, val) {
						if (val == "#") { return true; }
						if ($(val).length && $(val)
							.is(".jstree-closed")) { current.push(val); }
			...
			_get_next: function(obj, strict) {
				...
				if (obj.hasClass("jstree-open")) { return obj.find("li:eq(0)"); }
					...
					return obj.parentsUntil(".jstree", "li")
					.next("li")
					.eq(0);
				}
			},
			_get_prev: function(obj, strict) {
				...
				if (!obj.length) { return false; }
				if (strict) {
					return (obj.prevAll("li").length > 0) ? obj.prevAll(
						"li:eq(0)") : false;
				}
			...
			open_node: function(obj, callback, skip_animation) {
				...
				if (!this._is_loaded(obj)) {
					...
					if (s) {
						obj.children("ul")
						.stop(true)
						.slideDown(s, function() { this.style.display = ""; });
					}
			...
			close_node: function(obj, skip_animation) {
				...
				if (s) {
					obj.children("ul")
					.stop(true)
					.slideUp(s, function() { this.style.display = ""; });
				}
				this.__callback({"obj": obj});
			},
			...
			load_node: function(
				obj, s_call, e_call) { this.__callback({"obj": obj}); },
			...
			create_node: function(obj, position, js, callback, is_loaded) {
				...
				if (tmp === -1 || tmp.get(0) === this.get_container()
					.get(0)) { tmp = -1; }
				this.clean_node(tmp);
			...
			get_text: function(obj) {
				...
				if (s) {
					...
					obj = obj.contents()
					.filter(function() { return this.nodeType == 3; })[0];
			...
			delete_node: function(obj) {
				...
				if (p !== -1 && p.find("> ul > li").length === 0) {
					p.removeClass("jstree-open jstree-closed")
					.addClass("jstree-leaf");
				}
				this.clean_node(p);
			...
			move_node: function(
				...
				try {
					...
					if (!obj.op.find("> ul > li").length) {
						obj.op.removeClass("jstree-open jstree-closed")
						.addClass("jstree-leaf")
						.children("ul")
						.remove();
					}
		...
		__init: function() {
			...
			.bind("close_node.jstree", $.proxy(function(event, data) {
				var s = this._get_settings().ui,
					...
					clk = (obj && obj.length) ? obj.children("ul")
					.find(".jstree-clicked") : $(),
					_this = this;
			...
			.bind("move_node.jstree", $.proxy(function(event, data) {
				if (data.rslt.cy) {
					data.rslt.oc.find(".jstree-clicked")
		...
		_fn: {
			...
			save_selected: function() {
				...
				this.data.ui.selected.each(function() {
					_this.data.ui.to_select.push("#" + this.id.toString()
					.replace(/^#/, "")
					.replace('\\/', '/')
					.replace('/', '\\/'));
				});
			...
			select_node: function(obj, check, e) {
				...
				var s = this._get_settings().ui,
					...
					proceed = true;
				if (check) {
					if (s.disable_selecting_children && is_multiple && obj.parents(
							"li",
							this.get_container())
						.children(".jstree-clicked").length) {
						return false;
					}
					proceed = false;
					switch (!0) {
				...
				if (proceed && !is_selected) {
					obj.children("a").addClass("jstree-clicked round");
	...
	$.jstree.plugin("crrm", {
		...
		_fn: {
			_show_input: function(obj, callback) {
				...
				h2.width(Math.min(h1.text("pW" + h2[0].value).width(),
					w))[0].select();
			},
			rename: function(obj) {
				obj = this._get_node(obj);
			...
			check_move: function() {
				...
				if (!s.check_move.call(this,
						this._get_move())) { return false; }
				return true;
			},
			move_node: function(
				obj, ref, position, is_copy, is_prepared, skip_check) {
				...
				if (!is_prepared) {
					...
					if (position === "inside" && !s.default_position.match(
							/^(before|after)$/)) { position = s.default_position; }
					return this.__call_old(true,
						obj,
						ref,
						position,
						is_copy,
			...
			paste: function(obj) {
				...
				if (this.data.crrm.ct_nodes) {
					this.move_node(this.data.crrm.ct_nodes,
						obj);
				}
				if (this.data.crrm.cp_nodes) {
					this.move_node(this.data.crrm.cp_nodes,
						obj,
						false,
						true);
				}
		...
		_fn: {
			set_theme: function(theme_name, theme_url) {
				...
				if (this.data.themes.theme != theme_name) {
					this.get_container()
					.removeClass('jstree-' + this.data.themes.theme);
		...
		if ($.jstree._themes === false) {
			$("script").each(function() {
				if (this.src.toString()
					...
					$.jstree._themes = this.src.toString()
		...
		__init: function() {
			...
			$.each(this._get_settings().hotkeys, function(i, val) {
				if ($.inArray(i, bound) == -1) {
					...
					.bind("keydown",
						i,
						function(event) { return exec(i, event); });
					bound.push(i);
				}
			});
			this.enable_hotkeys();
		...
		_fn: {
			...
			_is_loaded: function(obj) {
				...
				if (obj && obj !== -1 && s.progressive_render && !obj.is(
					...
					.children("li").length === 0 && obj.data("jstree-children")) {
				...
				switch (!0) {
					...
					case (!s.data && !!s.ajax) || (!!s.data && !!s.ajax && obj && obj !== -1):
						error_func = function(x, t, e) {
							...
							if (obj != -1 && obj.length) {
								obj.children(".jstree-loading")
								.removeClass("jstree-loading");
								obj.data("jstree-is-loading", false);
								if (t === "success" && s.correct_state) {
									obj.removeClass("jstree-open jstree-closed")
									.addClass("jstree-leaf");
								}
							}
							else {
						...
						if ($.isFunction(s.ajax.url)) {
							s.ajax.url = s.ajax.url.call(this,
								obj);
						}
						if ($.isFunction(s.ajax.data)) {
							s.ajax.data = s.ajax.data.call(this,
								obj);
						}
						$.ajax(s.ajax);
						break;
				...
				if ($.isArray(js)) {
					...
					if (js.children) {
						if (s.progressive_render && js.state !== "open") {
							...
							.data("jstree-children", js.children);
						}
						else {
			...
			get_json: function(obj, li_attr, a_attr, is_callback) {
				...
				obj.each(function() {
					...
					$.each(li_attr, function(i, v) {
						tmp2 = li.attr(v);
						if (tmp2 && tmp2.length && tmp2.replace(
								/jstree[^ ]*|$/ig,
								'').length) {
					...
					a.each(function() {
						...
						if (
							...
							t.children("ins")
							.get(0).style.backgroundImage.length ||
							(t.children("ins").get(0).className && t.children(
								"ins")
							.get(0)
							.className
							...
							if ($.inArray("languages",
									s.plugins) !== -1 && $.isArray(s.languages) && s.languages.length) {
					...
					li = li.find("> ul > li");
					if (li.length) {
						tmp1.children = _this.get_json(li,
							li_attr,
							a_attr,
							true);
					}
	...
	$.jstree.plugin("sort", {
		__init: function() {
			...
			.bind("load_node.jstree", $.proxy(function(e, data) {
				...
				obj = obj === -1 ? this.get_container()
				.children("ul") : obj.children("ul");
				this.sort(obj);
			}, this))
		...
		defaults: function(
			a, b) { return this.get_text(a) > this.get_text(b) ? 1 : -1; },
		_fn: {
			sort: function(obj) {
...
(function($) {
	...
	$.vakata.dnd = {
		...
		drag: function(e) {
			...
			if (!$.vakata.dnd.is_drag) {
				if (Math.abs(e.pageX - $.vakata.dnd.init_x) > 5 || Math.abs(e.pageY - $.vakata.dnd.init_y) > 5) {
					...
					$(document)
					.triggerHandler("drag_start.vakata",
				...
				if (e.pageY - t < 20) {
					...
					if (!sti) {
						...
						sti = setInterval(function() {
							$(document)
							.scrollTop($(document)
							.scrollTop() - $.vakata.dnd.scroll_spd);
						}, 150);
					}
				}
				else {
				...
				if (e.pageX - l < 20) {
					...
					if (!sli) {
						...
						sli = setInterval(function() {
							$(document)
							.scrollLeft($(document)
							.scrollLeft() - $.vakata.dnd.scroll_spd);
						}, 150);
					}
				}
				else {
		...
		drag_stop: function(e) {
			...
			$(document)
			.triggerHandler("drag_stop.vakata",
				{"event": e, "data": $.vakata.dnd.user_data});
		...
		__init: function() {
			...
			.bind("mousemove.jstree", $.proxy(function(e) {
				if ($.vakata.dnd.is_drag && $.vakata.dnd.user_data.jstree) {
					...
					if (e.pageX + 24 > this.data.dnd.cof.left + this.data.dnd.cw) {
						if (this.data.dnd.i1) { clearInterval(this.data.dnd.i1); }
						this.data.dnd.i1 = setInterval($.proxy(function() { this.scrollLeft += $.vakata.dnd.scroll_spd; },
							cnt), 100);
					}
					...
					if (e.pageY + 24 > this.data.dnd.cof.top + this.data.dnd.ch) {
						if (this.data.dnd.i2) { clearInterval(this.data.dnd.i2); }
						this.data.dnd.i2 = setInterval($.proxy(function() { this.scrollTop += $.vakata.dnd.scroll_spd; },
							cnt), 100);
					}
			...
			.bind("drag_start.vakata", $.proxy(function(e, data) {
				if (data.data.jstree) {
					...
					if (et.closest(".jstree")
						.hasClass("jstree-" + this.get_index())) {
						this.dnd_enter(et);
					}
				}
			}, this));
						...
						$.vakata.dnd.drag_start(e,
							{
								jstree: true,
								obj: e.target
							}, "<ins class='jstree-icon'></ins>" + $(e.target)
						.text());
						if (this.data.themes) {
							m.attr("class", "jstree-" + this.data.themes.theme);
							$.vakata.dnd.helper.attr("class", "jstree-dnd-helper jstree-" + this.data.themes.theme);
			...
			if (s.drop_target) {
				...
				.delegate(s.drop_target,
					"mouseenter.jstree",
					$.proxy(function(e) {
						if (this.data.dnd.active && this._get_settings()
							.dnd
							.drop_check
							.call(this, {"o": o, "r": $(e.target)})) {
							$.vakata.dnd.helper.children("ins")
							.attr("class", "jstree-ok");
						}
					}, this))
				.delegate(s.drop_target,
					"mouseleave.jstree",
		...
		_fn: {
			...
			dnd_show: function() {
				...
				if (this.data.dnd.w < this.data.core.li_height / 3) {
					...
					o = this.data.dnd.w < this.data.core.li_height / 2 ? [
						"inside",
						"before",
						"after"
					] : ["inside", "after", "before"];
			...
			dnd_enter: function(obj) {
				...
				if (s.check_timeout) {
					...
					this.data.dnd.to1 = setTimeout($.proxy(this.dnd_prepare,
				...
				if (s.open_timeout) {
					...
					if (r && r.length && r.hasClass("jstree-closed")) {
						...
						this.data.dnd.to2 = setTimeout($.proxy(this.dnd_open,
							this), s.open_timeout);
					}
				}
				else {
				...
				$.vakata.dnd.drag_start(e,
					{
						jstree: true,
						obj: o
					}, "<ins class='jstree-icon'></ins>" + (o.length > 1 ? "Multiple selection" : this.get_text(
					o)));
				if (this.data.themes) {
					m.attr("class", "jstree-" + this.data.themes.theme);
					$.vakata.dnd.helper.attr("class", "jstree-dnd-helper jstree-" + this.data.themes.theme);
	...
	$.jstree.plugin("checkbox", {
		__init: function() {
			...
			.delegate("a", "click.jstree", $.proxy(function(e) {
				if (this._get_node(e.target)
					.hasClass("jstree-checked")) { this.uncheck_node(e.target); }
		...
		_fn: {
			_prepare_checkboxes: function(obj) {
				...
				if (obj.is("li")) { this._repair_state(obj); }
					...
					obj.find("> ul > li")
					.each(function() { _this._repair_state(this); });
				}
			},
...
(function($) {
	$.jstree.plugin("cookies", {
		__init: function() {
			...
			this.get_container()
			.one(( this.data.ui ? "reselect" : "reopen" ) + ".jstree", $.proxy(
				function() {
					this.get_container()
					.bind(
	...
	$.jstree.plugin("search", {
		...
		_fn: {
			search: function(str, skip_async) {
				...
				if (!skip_async && s.ajax !== false && this.get_container()
					.find(".jstree-closed:eq(0)").length > 0) {
				...
				.find("a" + (this.data.languages ? "." + this.get_lang() : "" ) + ":" + (s.case_insensitive ? "jstree_contains" : "contains") + "(" + this.data.search.str + ")");
			...
			_search_open: function(is_callback) {
				...
				if (this.data.search.to_open.length) {
					...
					if (current.length) {
						...
						$.each(current, function(i, val) {
							_this.open_node(val,
								function() { _this._search_open(true); });
						});
						done = false;
					}
				}
...
(function($) {
	...
	$(function() {
		...
		.delegate("a", "mouseup", function(e) {
			if (!$(this)
				.parent()
				.hasClass("jstree-contextmenu-disabled") && $.vakata.context.exec(
					$(this).attr("rel"))) {
				$.vakata.context.hide();
			}
			else { $(this).blur(); }
		...
		.delegate("a", "mouseover", function() {
			$.vakata.context.cnt.find(".vakata-hover")
		...
		.bind("mousedown",
			function(e) {
				if ($.vakata.context.vis && !$.contains($.vakata.context.cnt[0],
						e.target)) { $.vakata.context.hide(); }
			});
		if (typeof $.hotkeys !== "undefined") {
			$(document)
			.bind("keydown", "up", function(e) {
				if ($.vakata.context.vis) {
					var o = $.vakata.context.cnt.find("ul:visible")
					.last()
					.children(".vakata-hover")
					.removeClass("vakata-hover")
					.prevAll("li:not(.vakata-separator)")
					.first();
					if (!o.length) {
						o = $.vakata.context.cnt.find("ul:visible")
						.last()
						.children("li:not(.vakata-separator)")
						.last();
					}
	...
	$.jstree.plugin("types", {
			...
			.bind("init.jstree", $.proxy(function() {
				...
				$.each(types, function(i, tp) {
					$.each(tp, function(k, v) {
						if (!/^(max_depth|max_children|icon|valid_children)$/.test(
								k)) { _this.data.types.attach_to.push(k); }
					});
		...
		_fn: {
			...
			set_type: function(str, obj) {
				obj = this._get_node(obj);
				return (!obj.length || !str) ? false : obj.attr(this._get_settings().types.type_attr,
					str);
			},
			...
			check_move: function() {
				...
				if ($.isArray(vc) && m.ot && m.ot._get_type) {
					m.o.each(function() {
						if ($.inArray(m.ot._get_type(this),
								vc) === -1) {
							d = false;
							return false;
						}
				...
				if (s.max_children !== -2 && mc !== -1) {
					ch = m.cr === -1 ? this.get_container()
					.children("> ul > li")
					.not(m.o).length : m.cr.children("> ul > li")
					     .not(m.o).length;
			...
			create_node: function(
				obj, position, js, callback, is_loaded, skip_check) {
				if (!skip_check && (is_loaded || this._is_loaded(obj))) {
					var p = (position && position.match(/^before|after$/i) && obj !== -1) ? this._get_parent(
						obj) : this._get_node(obj),
						s = this._get_settings().types,
					...
					if ($.isArray(vc)) {
						if (!js.attr || !js.attr[s.type_attr]) {
							...
							if ($.inArray(js.attr[s.type_attr],
									vc) === -1) { return false; }
						}
					}
					if (s.max_children !== -2 && mc !== -1) {
						ch = p === -1 ? this.get_container()
						.children("> ul > li").length : p.children("> ul > li").length;
				...
				switch (!0) {
					case (!s.data && !s.ajax):
						if (!obj || obj == -1) {
							...
							.find("li, a")
							.filter(function() { return this.firstChild.tagName !== "INS"; })
		...
		_fn: {
			_themeroller: function(obj) {
				...
				obj = !obj || obj == -1 ? this.get_container() : this._get_node(
					obj).parent();
				obj
				.find("li.jstree-closed > ins.jstree-icon")
				.removeClass(s.opened)
				.addClass("ui-icon " + s.closed)
				.end()
				.find("li.jstree-open > ins.jstree-icon")
				.removeClass(s.closed)
				.addClass("ui-icon " + s.opened)
				.end()
				.find("a")
				.addClass(s.item)
				.children("ins.jstree-icon")
				.addClass("ui-icon " + s.item_icon);
			}
		},
		defaults: {
	...
	$.jstree.plugin("unique", {
		...
		_fn: {
			_check_unique: function(nms, p) {
				...
				.replace(/,$/, "")
				.split(",");
				if ((cnms.length + nms.length) != cnms.concat(nms)
					.sort()
					.join(",,")
					.replace(/(,|^)([^,]+)(,,\2)+(,|$)/g, "$1$2$4")
					.replace(/,,+/g, ",")
			...
			check_move: function() {
				...
				if (p.o && p.o.length) {
					...
					return this._check_unique(nms,
						p.np.find("> ul > li").not(p.o));
				}
				return true;
			}
		}
